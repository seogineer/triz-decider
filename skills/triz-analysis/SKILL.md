...
4. **원리 조회** — 고른 유형을 한 번에 `separation --type`으로 조회하되, **가장 잘 맞는 유형을 먼저** 적는다. 여러 유형이면 `ranking`(빈도 내림차순)에서, 하나면 `related_principles`에서 상위 3~5개를 순서대로 고른다. 동률은 스크립트가 `--type` 순서대로 유형을 번갈아 정렬해 두었으므로 출력 순서를 따르고, 특정 유형의 원리를 넣으려고 순서를 건너뛰지 않는다.
