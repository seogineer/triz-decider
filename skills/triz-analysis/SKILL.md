...
**`warnings`에 `missing_row`가 있을 때** — 그 개선 파라미터의 행은 현재 데이터셋에 없다. 비어 있는 것과 다르다. "이 조합은 데이터에 없어 조회할 수 없다"고 사용자에게 밝히고, 값을 추측하지 않는다. 가능하면 근접한 다른 파라미터(예: 움직이는/고정된 쌍의 다른 쪽)로 재매핑을 제안한다.
