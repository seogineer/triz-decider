...
- [ ] README(ko/en), 매니페스트, 로컬 설치 검증, 공개 (README·매니페스트·로컬 설치 검증 완료, 공개는 GitHub 저장소 생성·`<owner>` 치환·태그 필요)

### 11.2 미결 사항

- [ ] 플러그인 슬러그 확정: `triz-decider` 유지 여부 (등재 후 변경 불가)
- [x] 기존 Next.js 저장소를 재활용할지, 플러그인 전용 신규 저장소로 분리할지
- [x] 행렬 셀 값 교차 검증에 사용할 공개 자료 선정 (DATA_SOURCES.md 2.2)
- [ ] 원리 설명의 기본 언어: 한국어 우선 작성 후 영어 번역 vs 영어 우선
