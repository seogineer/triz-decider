...
| 매핑 정확도 | 평가 케이스 30건 블라인드 실행: 모든 실행에서 Top-3 적중 100%, 원리 번호 환각 0건, 보류된 셀에 걸렸을 때 9/9 사용자에게 알림. 표본이 작고 기대값을 작성자가 정했으며 실행 간 변동이 있으므로 스모크 테스트로 보세요. **독립 특허 벤치마크(TRIZBench, 30건)에서는 Hit@3 10%로, 가장 흔한 정답 3쌍만 찍는 기준선(13%)보다 낮았습니다.** 사용자가 트레이드오프를 직접 말할 때 가장 잘 맞고, 해법만 설명한 글에서는 기대하기 어렵습니다 (`tests/eval/results-v0.1.md`) |
