...
**사용자가 트레이드오프를 직접 말할 때**("X를 개선하면 Y가 나빠진다") 가장 잘 맞습니다. 특허 초록처럼 해법만 설명한 글에서 모순을 찾아내는 용도는 아닙니다. 데이터 상태의 벤치마크 설명을 참고하세요.
...
TRIZ는 이 플러그인보다 훨씬 넓습니다. v0.1은 기술적 모순 경로(모순 행렬 → 40 발명 원리)만 처리합니다. 다음은 **하지 않습니다.**

- 모순을 알기 전의 문제 분석: 기능 분석, 트리밍, 근본 원인·숨은 자원 분석
- 76가지 표준해와 물질-장 분석
- ARIZ, 기술시스템 진화 법칙, 기능지향검색
- 물리적 모순과 구조화 인터뷰(v0.2 예정)
