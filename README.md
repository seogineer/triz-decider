...
| Separation principles | Five types and their related principles follow the MATRIZ TRIZ Knowledge Base (CC BY 4.0), taken from a published transcription; a direct check against the MATRIZ wiki is still to do. Questions and examples written for this project. Literature disagrees on these lists, so treat them as one reputable reading, not the only one |
